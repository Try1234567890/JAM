...
import com.github.trfiles.memory.systems.ValueEntry;
import com.github.trfiles.memory.systems.ValueKey;
import com.github.trfiles.memory.systems.ValueRetriever;
import com.github.trfiles.memory.systems.converter.MapsToSectionsConverter;
import com.github.trfiles.memory.systems.converter.SectionsToMapsConverter;
import com.github.utilities.validators.Preconditions;

import java.util.*;

public class MemorySection implements Section {
    protected final MapsToSectionsConverter toSections = new MapsToSectionsConverter(this);
    protected final SectionsToMapsConverter toMaps = new SectionsToMapsConverter(this);
    ...
    public MemorySection(Map<?, ?> values,
        ...
        // external references.
        this.root = Preconditions.parameterNotNull(root, "root");
        this.parent = Preconditions.parameterNotNull(parent, "parent");
        this.name = Preconditions.parameterNotNull(name, "name");
        this.absolutePath = createAbsolutePath(parent, name);
    ...
    public MemorySection(Section parent, String name) {
        this(new LinkedHashMap<>(), parent.getRoot(), name, parent);
    }

    public MemorySection(Section parent, String name, Map<?, ?> values) {
        this(values, parent.getRoot(), name, parent);
    }
    ...
    /*Methods used while FileConfigurations loading. Should not be accessed by external.*/
    protected void withValues(Map<String, Object> values) {
        this.values.clear();
        ...
        while ((i1 = path.indexOf(separator, i2 = i1 + 1)) != -1) {
            ...
            if (section.isEmpty()) {
                ...
                throw new UnexpectedValueType("The value at key " + key + " inside the path " + path + " is not a Section. " +
                        "The value at " + path + " does not exist.");
            }
    ...
    /**
     * Set the {@code value} at the {@code path} of this section.
     *
    ...
    private void setValue(String key, Object value) {
        if (value == null) {
            if (getConfigurationOptions().NULLS_VALUES_REMOVE_THE_ENTRY.get()) {
                // Remove the key-value pair from the map if the options is true
                values.remove(key);
            }
            // Returns anyway if the value is null to avoid a useless conversion.
            // In reality the conversion will not happen, because a null value cannot
            // be converted, but if we can return why not?.
            return;
        }
